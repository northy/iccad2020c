...
int main(int argc, char** argv) {
    ...
    platform.getDevices(CPU_GPU_DEF, &devices);
    ...
    if (err) { std::cout << "Program error: " << getErrorString(err) << " (" << err << ")\n"; return 1; }

    //BUFFERS
    char buf[3];
    cl::Buffer membuf(context, CL_MEM_WRITE_ONLY | CL_MEM_HOST_READ_ONLY, sizeof(buf), &err);
    if (err) { std::cout << "Buffer error: " << getErrorString(err) << " (" << err << ")\n"; return 1; }

    //KERNEL
    cl::Kernel kernel(program, "write", &err);
    kernel.setArg(0, membuf);
    if (err) { std::cout << "Kernel error: " << getErrorString(err) << " (" << err << ")\n"; return 1; }

    //COMMAND QUEUE
    cl::CommandQueue queue(context, device);
    queue.enqueueTask(kernel);
    queue.enqueueReadBuffer(membuf, CL_FALSE, 0, sizeof(buf), buf);
